...
- Browser smoke coverage exercises first setup, existing-device settings, photo source modes, date filtering, layout and metadata controls, clock and NTP controls, screen tone and night schedule controls, screen rotation developer safeguards, backup export/import, rejected import behavior, firmware update states, logs, and a mobile-width render check.
