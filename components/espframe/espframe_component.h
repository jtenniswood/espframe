...
class EspFrameComponent : public Component, public ConfigurationUpdateScheduler {
 public:
  ...
  template<typename... Scripts> void stop_slideshow_workers(Scripts *...scripts) { stop_scripts_in_order(scripts...); }
  ...
  SlotFetchDecision check_slot_fetch(bool wifi_connected, bool configured, bool paused,
    ...
    return slot_fetch_decision(this->slideshow_.state(), wifi_connected, configured, paused, cooldown, image_downloading);
  }
